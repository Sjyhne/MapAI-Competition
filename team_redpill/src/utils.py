...
class SimpleBuildingDataset(BuildingDataset):
...
class BuildingLidarDataset(Dataset):
    def __init__(self, lidar_paths, mask_paths, size=(256, 256), transform=None):
    ...
    def __getitem__(self, idx):
        lidar = np.array(Image.open(self.lidar_paths[idx]), dtype=np.float32)
        mask = np.array(Image.open(self.mask_paths[idx]))
        mask[mask > 0.0] = 1.0
...
class SimpleBuildingLidarDataset(BuildingLidarDataset):
    def __getitem__(self, *args, **kwargs):
        sample = super().__getitem__(*args, **kwargs)
        # resize images
        lidar = np.array(Image.fromarray(sample["lidar"]).resize((256, 256), Image.LINEAR))
        mask = np.array(Image.fromarray(sample["mask"]).resize((256, 256), Image.NEAREST))
        # convert to other format HWC -> CHW
        sample["lidar"] = np.expand_dims(lidar, 0)
        sample["mask"] = np.expand_dims(mask, 0)
        return sample
...
class LidarNet(pl.LightningModule):
    ...
    def shared_step(self, batch, stage):
        ...
        logits_mask = self.forward(lidar)
