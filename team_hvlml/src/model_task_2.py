...
def main(args):
    ...
    models_path = snapshot_download(repo_id="HVL-ML/MapAI-2022", allow_patterns="*.pkl")

    model_list = glob.glob(models_path+'/task_2*.pkl')
    ...
    for idx, (image, label, filename) in tqdm(enumerate(dataloader), total=len(dataloader), desc="Inference", leave=False):
        ...
        lidar_fn = f"../../data/task{opts['task']}_{opts['data_type']}/lidar/{filename[0]}"
        label_fn = f"../../data/task{opts['task']}_{opts['data_type']}/masks/{filename[0]}"

        #img = PILImage.create(img_fn)
        label = np.uint8(TensorMask(PILImage.create(label_fn))[:,:,0])
        imglidar_fn = img_fn + ";" + lidar_fn

        imglidar = MImage.create(custom_load_image(imglidar_fn))
        # Model prediction
        if not ensemble:
            pred,_,probs = learn.predict(imglidar)
            pred = torch.where(pred>0.5, 0, 1)
            prediction = np.uint8(pred)[0,::]
            ...
            for learn in learners:
                ...
                _,_,probs = learn.predict(imglidar)
        ...
        #ax[0].imshow(PILImage.create(img_fn))
